"""Git-backed protection for Butovsky Admin customization tool calls."""
...
CORE_GUARD_JOURNAL_SETTING = "butovsky_admin_core_guard_journal"
CORE_GUARD_REF_PREFIX = "refs/yaa-snapshots"
CORE_GUARD_TMP_DIRNAME = "butovsky_core_guard"
...
def _load_journal() -> dict[str, dict[str, Any]]:
    ...
    try:
        ...
        raise ValueError("invalid Butovsky Admin core guard journal JSON") from exc
    if not isinstance(payload, dict):
        raise ValueError("invalid Butovsky Admin core guard journal root")
    transactions = payload.get("transactions", {})
    if not isinstance(transactions, dict):
        raise ValueError("invalid Butovsky Admin core guard transactions map")
    return transactions
...
async def _prepare_transaction(
    ...
    *,
    ...
    if dirty:
        ...
        commit_env = {
            "GIT_AUTHOR_NAME": "Butovsky Admin Core Guard",
            "GIT_AUTHOR_EMAIL": "core-guard@butovsky.invalid",
            "GIT_COMMITTER_NAME": "Butovsky Admin Core Guard",
            "GIT_COMMITTER_EMAIL": "core-guard@butovsky.invalid",
        }
        commit_result = await _run_git(
            ...
            [
                ...
                baseline_head,
                "-m",
                f"Butovsky Admin checkpoint {request_id}/{token}",
            ],
    ...
    async with _repository_lock:
        ...
        try:
            ...
            try:
                ...
                logger.critical(
                    "Could not persist Butovsky Admin core guard failure: %s",
                    journal_error,
                )
            log = logger.critical if tool_executed else logger.error
            log(
                "Butovsky Admin core guard failure: request_id=%s tool_call_id=%s stage=%s executed=%s error=%s",
                request_id,
                tool_call_id,
                exc.stage,
                tool_executed,
...
async def finalize_core_guard(request_id: int, tool_call_id: str) -> bool:
    ...
    async with _repository_lock:
        ...
        try:
            ...
            logger.critical("Failed to finalize Butovsky Admin core guard: %s", exc)
            return False
        return True
        ...
        try:
            ...
            for key, entry in list(transactions.items()):
                ...
                if phase in {"preparing", "checkpointing", "preparation_failed"}:
                    if phase == "checkpointing":
                        try:
                            ...
                            logger.critical(
                                "Failed to clean interrupted Butovsky Admin checkpoint %s: %s",
                                key,
                                exc,
                            )
                            continue
                ...
                try:
                    ...
                    logger.warning(
                        "Recovered interrupted Butovsky Admin core guard: request_id=%s tool_call_id=%s reverted=%s",
                    ...
                    logger.critical("Failed to recover Butovsky Admin core guard %s: %s", key, exc)
        finally:
            _repository_guard_active -= 1
