...
@router.callback_query(F.data == "admin_send_log_to_butovsky")
async def send_log_to_butovsky_admin(callback: CallbackQuery, state: FSMContext):
    """Sends bot.log to Butovsky Admin for analysis."""
    if not is_admin(callback.from_user.id):
        await callback.answer("⛔ Доступ запрещён", show_alert=True)
        return
    ...
    await callback.answer()
    from bot.handlers.admin.butovsky_admin import (
        _ButovskyProgressRenderer,
        _activate_butovsky_chat_lane,
    ...
    status_message = await safe_edit_or_send(
        ...
        reply_markup=butovsky_admin_agent_kb(BUTOVSKY_ADMIN_CHAT_TOPIC_ID),
    )

    progress = _ButovskyProgressRenderer(
        status_message,
        topic_id=BUTOVSKY_ADMIN_CHAT_TOPIC_ID,
    )
    try:
        final = await run_dialog_with_uploads(
            ...
            [ButovskyAdminUpload(log_path, "bot.log", "text/plain")],
            topic_id=BUTOVSKY_ADMIN_CHAT_TOPIC_ID,
        ...
        await safe_edit_or_send(
            ...
            reply_markup=butovsky_admin_request_error_kb(
                BUTOVSKY_ADMIN_CHAT_TOPIC_ID,
                active_request=(
