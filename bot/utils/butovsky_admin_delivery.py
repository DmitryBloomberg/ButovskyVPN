"""Telegram delivery helpers for Butovsky Admin final responses."""
...
async def edit_or_send_butovsky_admin_final(
    message: Message,
    *,
    fallback_html: str,
    ...
    if rich_markdown:
        try:
            ...
            logger.debug("Butovsky Admin final delivered as Rich Markdown edit")
            return result if isinstance(result, Message) else message
        except (TelegramBadRequest, AttributeError, TypeError) as exc:
            logger.warning(
                "Butovsky Admin Rich Markdown edit rejected; using HTML: %s",
                str(exc)[:200],
            )
...
async def send_butovsky_admin_final(
    bot: Any,
    *,
    chat_id: int,
    fallback_html: str,
    ...
    if rich_markdown:
        try:
            ...
            logger.debug("Butovsky Admin final delivered as fresh Rich Markdown")
            return result
        except (TelegramBadRequest, AttributeError, TypeError) as exc:
            logger.warning(
                "Butovsky Admin Rich Markdown send rejected; using HTML: %s",
                str(exc)[:200],
            )
