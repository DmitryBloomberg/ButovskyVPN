...
__all__ = [
    ...
    'delete_butovsky_admin_api_key',
    'get_butovsky_admin_server_ip',
    'set_butovsky_admin_server_ip',
    'delete_butovsky_admin_server_ip',
...
BUTOVSKY_ADMIN_API_KEY_SETTING = 'butovsky_admin_api_key'
BUTOVSKY_ADMIN_SERVER_IP_SETTING = 'butovsky_admin_server_ip'
BUTOVSKY_ADMIN_CORE_CHANGES_ENABLED_SETTING = 'butovsky_admin_core_changes_enabled'
BUTOVSKY_ADMIN_REQUEST_SETTING_PREFIX = 'butovsky_admin_request'
BUTOVSKY_ADMIN_TOOL_CALL_SETTING_PREFIX = 'butovsky_admin_tool_call'
BUTOVSKY_ADMIN_TOOL_RUNTIME_SETTING_PREFIX = 'butovsky_admin_tool_runtime'


def get_butovsky_admin_api_key() -> Optional[str]:
    """Returns the general Butovsky Admin api_key for this Telegram bot."""
    return get_setting(BUTOVSKY_ADMIN_API_KEY_SETTING)


def set_butovsky_admin_api_key(api_key: str) -> None:
    """Saves the general api_key Butovsky Admin in settings."""
    set_setting(BUTOVSKY_ADMIN_API_KEY_SETTING, api_key)


def delete_butovsky_admin_api_key() -> bool:
    """Removes the general api_key Butovsky Admin from settings."""
    return delete_setting(BUTOVSKY_ADMIN_API_KEY_SETTING)


def get_butovsky_admin_server_ip() -> str:
    """Returns the saved public IP of the server for Butovsky Admin."""
    return get_setting(BUTOVSKY_ADMIN_SERVER_IP_SETTING, '') or ''


def set_butovsky_admin_server_ip(server_ip: str) -> None:
    """Saves the public IP of the server for Butovsky Admin in settings."""
    set_setting(BUTOVSKY_ADMIN_SERVER_IP_SETTING, server_ip.strip())


def delete_butovsky_admin_server_ip() -> bool:
    """Removes the saved public IP of the Butovsky Admin server from settings."""
    return delete_setting(BUTOVSKY_ADMIN_SERVER_IP_SETTING)


def is_butovsky_admin_core_changes_enabled() -> bool:
    """Return the hidden flag that allows core/source changes via Butovsky Admin."""
    return get_setting(BUTOVSKY_ADMIN_CORE_CHANGES_ENABLED_SETTING, '0') == '1'


def _butovsky_admin_request_key(kind: str, telegram_id: int, topic_id: int) -> str:
    """Settings key for request_id in lane Butovsky Admin."""
    return (
        f'{BUTOVSKY_ADMIN_REQUEST_SETTING_PREFIX}:'
        f'{kind}:{int(telegram_id)}:{int(topic_id)}'
    )


def _get_butovsky_admin_request_id(kind: str, telegram_id: int, topic_id: int) -> Optional[int]:
    """Reads request_id Butovsky Admin from settings."""
    raw = get_setting(_butovsky_admin_request_key(kind, telegram_id, topic_id))
    if not raw:
        return None
    try:
        return int(raw)
...
def get_butovsky_admin_active_request_id(telegram_id: int, topic_id: int) -> Optional[int]:
    """Returns active request_id Butovsky Admin from settings."""
    return _get_butovsky_admin_request_id('active', telegram_id, topic_id)


def set_butovsky_admin_active_request_id(telegram_id: int, topic_id: int, request_id: int) -> None:
    """Saves active request_id Butovsky Admin in settings."""
    set_setting(
        _butovsky_admin_request_key('active', telegram_id, topic_id),
        str(int(request_id)),
    )


def _clear_butovsky_admin_request_id(
    kind: str,
    telegram_id: int,
    topic_id: int,
    *,
    ...
    key = _butovsky_admin_request_key(kind, telegram_id, topic_id)
...
def clear_butovsky_admin_active_request_id(
    telegram_id: int,
    topic_id: int,
    *,
    expected_request_id: Optional[int] = None,
    ...
    return _clear_butovsky_admin_request_id(
        'active',
        telegram_id,
        topic_id,
...
def list_butovsky_admin_active_requests() -> List[Dict[str, int]]:
    """Returns all saved active request_ids by lane."""
    prefix = f'{BUTOVSKY_ADMIN_REQUEST_SETTING_PREFIX}:active:'
    with get_db() as conn:
        rows = conn.execute(
...
def get_butovsky_admin_last_request_id(telegram_id: int, topic_id: int) -> Optional[int]:
    """Returns last request_id Butovsky Admin from settings."""
    return _get_butovsky_admin_request_id('last', telegram_id, topic_id)


def set_butovsky_admin_last_request_id(telegram_id: int, topic_id: int, request_id: int) -> None:
    """Saves last request_id Butovsky Admin in settings."""
    set_setting(
        _butovsky_admin_request_key('last', telegram_id, topic_id),
        str(int(request_id)),
    )


def clear_butovsky_admin_last_request_id(
    telegram_id: int,
    topic_id: int,
    *,
    expected_request_id: Optional[int] = None,
...
def _butovsky_admin_tool_call_key(request_id: int, tool_call_id: str) -> str:
    """The settings key for a locally started tool_call."""
    return (
        f'{BUTOVSKY_ADMIN_TOOL_CALL_SETTING_PREFIX}:'
        f'{int(request_id)}:{tool_call_id}'
    )


def mark_butovsky_admin_tool_call_started(request_id: int, tool_call_id: str) -> bool:
    """
    ...
    """
    key = _butovsky_admin_tool_call_key(request_id, tool_call_id)
    if get_setting(key):
        return False
...
def clear_butovsky_admin_tool_call_started(request_id: int, tool_call_id: str) -> bool:
    """Removes the started flag from tool_call after the result has been successfully sent."""
    return delete_setting(_butovsky_admin_tool_call_key(request_id, tool_call_id))


def _butovsky_admin_tool_runtime_key(request_id: int, tool_call_id: str) -> str:
    """The settings key for the runtime state of the running tool_call."""
    return (
        f'{BUTOVSKY_ADMIN_TOOL_RUNTIME_SETTING_PREFIX}:'
        f'{int(request_id)}:{tool_call_id}'
    )


def set_butovsky_admin_tool_runtime(
    request_id: int,
    tool_call_id: str,
    tool: str,
    *,
    ...
    set_setting(
        _butovsky_admin_tool_runtime_key(request_id, tool_call_id),
        json.dumps(payload, ensure_ascii=False, separators=(',', ':')),
    )


def clear_butovsky_admin_tool_runtime(request_id: int, tool_call_id: str) -> bool:
    """Removes the tool_call runtime marker after the result is sent normally."""
    return delete_setting(_butovsky_admin_tool_runtime_key(request_id, tool_call_id))


def list_butovsky_admin_tool_runtime(
    request_id: Optional[int] = None,
    topic_id: Optional[int] = None,
) -> List[Dict[str, Any]]:
    """Returns runtime markers of locally executed tool_calls."""
    prefix = f'{BUTOVSKY_ADMIN_TOOL_RUNTIME_SETTING_PREFIX}:'
    with get_db() as conn:
        rows = conn.execute(
            "SELECT key, value FROM settings WHERE key LIKE ?",
            (f'{prefix}%',),
