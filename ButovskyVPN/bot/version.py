"""Runtime identity of the installed ButovskyVPN bot release."""
...
def resolve_bot_version() -> tuple[str, str]:
    ...
    if not success or "\t" not in output:
        logger.warning("Cannot resolve ButovskyVPN bot release from Git HEAD")
    ...
    if release is None:
        logger.warning(
            "Current Git commit subject has no ButovskyVPN release prefix: %s",
            subject[:160],
        )
