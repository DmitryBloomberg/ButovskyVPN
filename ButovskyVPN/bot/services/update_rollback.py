...
SERVICE_NAME = "butovsky-vpn"
UPDATER_SERVICE_TEMPLATE = "butovsky-vpn-updater@"
...
def _repository_guard_is_active() -> bool:
    """Return the in-process Butovsky Admin repository-fence state."""
    try:
        from bot.services.butovsky_admin_core_guard import is_repository_guard_active
    except ImportError:
        return False
...
def install_registered_updater_service(
    *,
    ...
    runner_placeholder = "__BUTOVSKY_SYSTEMD_INSTANCE__"
    runner_path = (
        root
        / "backup"
        / PRE_UPDATE_DIRNAME
...
def schedule_admin_update(
    *,
    ...
    if _repository_guard_is_active():
        return False, (
            "Обновление временно недоступно: Butovsky Admin проверяет изменения "
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="ButovskyVPN update rollback manager")
