"""
Butovsky Admin satellite protocol client.
...
from bot.services.butovsky_admin_core_guard import (
    finalize_core_guard,
    finalize_core_guards_for_request,
    interrupted_tool_result,
    run_with_core_guard,
)
from bot.services.butovsky_admin_page_binding import (
    YaaPageBinding,
...
from bot.services.butovsky_admin_customization_tools import (
...
from database.requests import (
    clear_butovsky_admin_active_request_id,
    clear_butovsky_admin_last_request_id,
    clear_butovsky_admin_tool_call_started,
    clear_butovsky_admin_tool_runtime,
    get_butovsky_admin_active_request_id,
    get_butovsky_admin_api_key,
    get_butovsky_admin_last_request_id,
    get_butovsky_admin_server_ip,
    is_butovsky_admin_core_changes_enabled,
    list_butovsky_admin_active_requests,
    list_butovsky_admin_tool_runtime,
    mark_butovsky_admin_tool_call_started,
    set_butovsky_admin_active_request_id,
    set_butovsky_admin_last_request_id,
    set_butovsky_admin_server_ip,
    set_butovsky_admin_tool_runtime,
)

logger = logging.getLogger(__name__)

HUB_URLS: tuple[str, ...] = (
    "https://admin.t.me/ButovskyLive",
    "https://admin-en.t.me/ButovskyLive",
)
...
UPLOAD_TMP_DIR = TMP_DIR / "butovsky_uploads"
BUTOVSKY_ADMIN_CHAT_TOPIC_ID = 0
BUTOVSKY_ADMIN_YAA_TOPIC_ID = 1001
BUTOVSKY_ADMIN_CUSTOMIZATION_TOPIC_ID = 1002
BUTOVSKY_ADMIN_BROADCAST_TOPIC_ID = 1003
BUTOVSKY_ADMIN_DEFAULT_SKILL_ID = "butovsky_vpn"
BUTOVSKY_ADMIN_CUSTOMIZATION_SKILL_ID = "butovsky_vpn_customization"
BUTOVSKY_ADMIN_BROADCAST_SKILL_ID = "butovsky_vpn_broadcast"
BUTOVSKY_ADMIN_SATELLITE_TYPE = "butovsky_vpn"
...
BUTOVSKY_ADMIN_TELEGRAM_HTML_TASK_FORMAT = "telegram_html"
...
_BUTOVSKY_ADMIN_API_KEY_RE = re.compile(
    r"[A-Za-z0-9\-._~+/]+=*",
    flags=re.ASCII,
)
PUBLIC_IP_URLS = (
...
class _HubEndpointSelector:
    ...
    def mark_success(self, endpoint: str) -> None:
        try:
            ...
            logger.warning("Ignoring an unknown Butovsky Admin Hub endpoint")
...
_SELF_RESTART_RE = re.compile(
    r"^\s*(?:sudo\s+)?(?:"
    r"systemctl\s+(?:restart|try-restart)\s+butovsky-vpn(?:\.service)?"
    r"|service\s+butovsky-vpn\s+restart"
    r")\s*$",
    flags=re.IGNORECASE,
)


def is_butovsky_admin_customization_topic(topic_id: int) -> bool:
    """Return True for Butovsky Admin lanes that use the customization skill."""
    return int(topic_id) in {
        BUTOVSKY_ADMIN_YAA_TOPIC_ID,
        BUTOVSKY_ADMIN_CUSTOMIZATION_TOPIC_ID,
    }


def butovsky_admin_message_storage_format(
    topic_id: int,
) -> Literal["html", "plain"]:
    """Return the Telegram message representation sent to the Hub."""
    return "html" if is_butovsky_admin_customization_topic(topic_id) else "plain"


def butovsky_admin_task_format_for_topic(topic_id: int) -> str | None:
    """Return runtime metadata for the current user message, when required."""
    if butovsky_admin_message_storage_format(topic_id) == "html":
        return BUTOVSKY_ADMIN_TELEGRAM_HTML_TASK_FORMAT
    return None


def is_butovsky_admin_broadcast_topic(topic_id: int) -> bool:
    """Return True only for the structured broadcast editor lane."""
    return int(topic_id) == BUTOVSKY_ADMIN_BROADCAST_TOPIC_ID
...
def _capabilities_for_skill(
    ...
    *,
    ...
    if skill_id == BUTOVSKY_ADMIN_BROADCAST_SKILL_ID:
        ...
        if (
            skill_id == BUTOVSKY_ADMIN_CUSTOMIZATION_SKILL_ID
...
def butovsky_admin_skill_id_for_topic(
    topic_id: int,
    requested_skill_id: Optional[str] = None,
) -> str:
    """Resolve the skill id sent to the hub for a local Butovsky Admin lane."""
    requested = (requested_skill_id or "").strip()
    if is_butovsky_admin_broadcast_topic(topic_id):
        return BUTOVSKY_ADMIN_BROADCAST_SKILL_ID
    if requested in {
        BUTOVSKY_ADMIN_DEFAULT_SKILL_ID,
        BUTOVSKY_ADMIN_CUSTOMIZATION_SKILL_ID,
    }:
        return requested
    if is_butovsky_admin_customization_topic(topic_id):
        return BUTOVSKY_ADMIN_CUSTOMIZATION_SKILL_ID
    return BUTOVSKY_ADMIN_DEFAULT_SKILL_ID


def _core_policy_for_skill(skill_id: str) -> Optional[bool]:
    """Return the core-change policy payload for customization skill calls."""
    if skill_id != BUTOVSKY_ADMIN_CUSTOMIZATION_SKILL_ID:
        return None
    return is_butovsky_admin_core_changes_enabled()
...
def build_agent_env_context() -> dict[str, Any]:
    ...
    try:
        ...
        logger.warning("Не удалось собрать версию БД для Butovsky Admin context: %s", e)
    ...
    try:
        ...
        logger.warning("Не удалось собрать статус custom extensions для Butovsky Admin context: %s", e)
...
def _runtime_context_factory_for_turn(
    ...
    task_format = butovsky_admin_task_format_for_topic(topic_id)
    binding = page_binding or get_yaa_page_binding(telegram_id, topic_id)
    if topic_id == BUTOVSKY_ADMIN_YAA_TOPIC_ID and binding is not None:
...
def normalize_butovsky_admin_api_key(api_key: str) -> str:
    ...
    if any(
        ...
        raise ValueError("invalid api_key format")
    if _BUTOVSKY_ADMIN_API_KEY_RE.fullmatch(normalized) is None:
        raise ValueError("invalid api_key format")
    return normalized
...
def _hub_headers(api_key: str) -> dict[str, str]:
    ...
    try:
        normalized_api_key = normalize_butovsky_admin_api_key(api_key)
    except ValueError:
        logger.warning(
            "Butovsky Admin request rejected locally: invalid api_key format"
        )
        raise ButovskyAdminError(
            "Butovsky Admin request rejected locally: invalid api_key format",
            kind="configuration",
    ...
    return {
        ...
        "X-Butovsky-Satellite-Protocol-Version": SATELLITE_PROTOCOL_VERSION,
    }


class ButovskyAdminError(RuntimeError):
    """Classified failure of a Butovsky Admin request."""
...
class ButovskyAdminRequestStopped(ButovskyAdminError):
...
@dataclass
class ButovskyAdminProgressEvent:
...
@dataclass
class ButovskyAdminUpload:
    """File to be sent to Butovsky Admin upload API."""
...
@dataclass
class ButovskyAdminLatest:
    """Latest non-destructive snapshot of the request."""

    request_id: int
    event: str = ""
    final: Optional[ButovskyAdminFinal] = None
    progress: Optional[ButovskyAdminProgressEvent] = None
    resume_allowed: bool = True
    cancel_button_text: str | None = None


@dataclass
class ButovskyAdminNewChatResult:
...
@dataclass
class ButovskyAdminCancelResult:
...
ProgressCallback = Callable[[ButovskyAdminProgressEvent], Awaitable[None]]
...
def _lane_key(
    telegram_id: int,
    topic_id: int = BUTOVSKY_ADMIN_CHAT_TOPIC_ID,
) -> RequestLaneKey:
...
def get_active_request_id(
    ...
    return _active_requests.get(key) or get_butovsky_admin_active_request_id(
        telegram_id,
        topic_id,
    )
...
def get_last_request_id(
    ...
    key = _lane_key(telegram_id, topic_id)
    return _last_requests.get(key) or get_butovsky_admin_last_request_id(
        telegram_id,
        topic_id,
    )
...
def _remember_request(
    ...
    *,
    ...
    set_butovsky_admin_last_request_id(telegram_id, topic_id, request_id)
    if active:
        _active_requests[key] = request_id
        set_butovsky_admin_active_request_id(telegram_id, topic_id, request_id)
...
def _clear_tool_runtime(request_id: int, tool_call_id: str) -> None:
    ...
    clear_butovsky_admin_tool_runtime(request_id, tool_call_id)
...
def get_local_tool_diagnostics(
    ...
    for payload in list_butovsky_admin_tool_runtime(request_id, topic_id):
...
def _core_guard_enabled(topic_id: int) -> bool:
    return (
        is_butovsky_admin_customization_topic(topic_id)
        and not is_butovsky_admin_core_changes_enabled()
    )
...
async def _get_server_ip(session: aiohttp.ClientSession) -> str:
    ...
    saved_ip = get_butovsky_admin_server_ip().strip()
    if saved_ip:
        return saved_ip
    ...
    if detected_ip:
        try:
            set_butovsky_admin_server_ip(detected_ip)
        except Exception as e:
            logger.warning("Не удалось сохранить публичный IP Butovsky Admin: %s", e)
    return detected_ip


def _is_retryable_hub_error(error: Exception) -> bool:
    """Return whether repeating the same HTTP operation may recover."""
    if not isinstance(error, ButovskyAdminError):
        return True
    if error.kind in {
        "authentication",
        "configuration",
...
def _can_repeat_hub_request(error: Exception, *, read_only: bool) -> bool:
    ...
    if read_only:
        if isinstance(error, ButovskyAdminError) and error.kind == "protocol":
            return True
...
async def _request_json(
    ...
    *,
    ...
    for attempt, endpoint in enumerate(endpoints, start=1):
        ...
        except (aiohttp.ClientError, asyncio.TimeoutError, ButovskyAdminError) as e:
            last_error = e
            if not _can_repeat_hub_request(e, read_only=read_only):
                if isinstance(e, ButovskyAdminError):
                    raise
                raise ButovskyAdminError(
                    f"Could not reach the Butovsky Admin hub: {e}",
                ) from e
            if attempt >= max_attempts:
                break
            delay = delays[min(attempt - 1, len(delays) - 1)]
            logger.warning(
                "Ошибка запроса к Butovsky Admin (%s %s via %s), попытка %s/%s: %s",
                method,
                path,
    ...
    if isinstance(last_error, ButovskyAdminError):
        raise last_error
    raise ButovskyAdminError(
        f"Could not reach the Butovsky Admin hub: {last_error}",
    ) from last_error


def _incompatible_broadcast_hub(detail: str) -> ButovskyAdminError:
    """Build a stable user-facing error for a missing safe editor contract."""
    return ButovskyAdminError(
        f"incompatible broadcast editor hub: {detail}",
        user_message=(
            "Редактор рассылок пока несовместим с хабом. "
            "Сначала обновите Butovsky Admin на хабе, затем повторите /yaa. "
            "Обычная рассылка при этом продолжает работать."
        ),
    )


def _incompatible_customization_hub(detail: str) -> ButovskyAdminError:
    ...
    return ButovskyAdminError(
        ...
        user_message=(
            ...
            "Проверьте подключение и API-ключ типа ButovskyVPN, затем повторите запрос."
        ),
    )
...
def _is_temporary_hub_unavailability(error: ButovskyAdminError) -> bool:
...
async def _ensure_broadcast_hub_support(
    ...
    except ButovskyAdminError as error:
    ...
    if data.get("satellite_type") != BUTOVSKY_ADMIN_SATELLITE_TYPE:
...
async def _negotiate_runtime_context_support(
    ...
    supported = bool(
        ...
        and data.get("satellite_type") == BUTOVSKY_ADMIN_SATELLITE_TYPE
    ...
    if skill_id == BUTOVSKY_ADMIN_CUSTOMIZATION_SKILL_ID and not supported:
...
def _validate_specialized_hub_response(data: dict[str, Any], skill_id: str) -> None:
    ...
    if skill_id not in {
        BUTOVSKY_ADMIN_CUSTOMIZATION_SKILL_ID,
        BUTOVSKY_ADMIN_BROADCAST_SKILL_ID,
    }:
        return
    if data.get("satellite_type") != BUTOVSKY_ADMIN_SATELLITE_TYPE:
...
async def _request_multipart(
    ...
    *,
    fields: dict[str, Any],
    uploads: list[ButovskyAdminUpload],
    file_field: str,
) -> tuple[int, Optional[dict]]:
        ...
        try:
            ...
            if not _can_repeat_hub_request(e, read_only=False):
                if isinstance(e, ButovskyAdminError):
                    raise
                raise ButovskyAdminError(
                    f"Could not upload a file to the Butovsky Admin hub: {e}",
                ) from e
            if attempt >= max_attempts:
                break
            delay = delays[min(attempt - 1, len(delays) - 1)]
            logger.warning(
                "Ошибка upload-запроса к Butovsky Admin (%s via %s), попытка %s/%s: %s",
                path,
                urlsplit(endpoint).netloc,
                attempt,
                max_attempts,
    ...
    if isinstance(last_error, ButovskyAdminError):
        raise last_error
    raise ButovskyAdminError(
        f"Could not upload a file to the Butovsky Admin hub: {last_error}",
    ) from last_error
...
def _log_tool_audit(event: dict[str, Any], tool_result: dict[str, Any]) -> None:
    ...
    if tool == "satellite_sql":
        logger.info(
            "Butovsky Admin tool audit: request_id=%s tool_call_id=%s tool=%s "
...
async def _run_tool_call(
    ...
    *,
    ...
    if tool == "satellite_broadcast_editor":
        ...
        _log_tool_audit(event, result)
        return result
    if tool in CUSTOMIZATION_TOOL_NAMES and not is_butovsky_admin_customization_topic(topic_id):
        result = {
            "result": "",
            "error": (
    ...
    async def execute_tool() -> dict[str, Any]:
        if _is_deferred_self_restart(tool, args):
            return {
                "result": (
                    "ButovskyVPN service restart queued. It will run only after the "
...
async def _notify_progress(
    ...
    progress_event = ButovskyAdminProgressEvent(
    ...
    try:
        ...
        logger.warning(
            "Не удалось показать progress-событие Butovsky Admin: event=%s slot=%s error=%s",
...
async def _schedule_deferred_self_restart() -> None:
    ...
    if os.name == "nt":
        logger.warning("Deferred ButovskyVPN self-restart is unavailable on Windows")
        return
    try:
        await asyncio.create_subprocess_exec(
            "/bin/bash",
            "-c",
            "sleep 1; systemctl restart butovsky-vpn.service",
            stdin=asyncio.subprocess.DEVNULL,
            stdout=asyncio.subprocess.DEVNULL,
            stderr=asyncio.subprocess.DEVNULL,
            start_new_session=True,
        )
        logger.info("ButovskyVPN self-restart scheduled after tool_result delivery")
    except OSError as exc:
        logger.error("Failed to schedule deferred ButovskyVPN self-restart: %s", exc)
...
async def _poll_until_final(
    ...
    *,
    ...
    logger.info(
        "Butovsky Admin request accepted: admin=%s topic=%s request_id=%s satellite_type=%s server_ip=%s",
        telegram_id,
        topic_id,
        request_id,
        satellite_type,
    ...
    try:
        while True:
            ...
            if cycle.poll_stop_requested.is_set() or poll_result is None:
                ...
                raise ButovskyAdminRequestStopped(request_id)
            status_code, event = poll_result
            if status_code == 204:
                continue
            if not event:
                raise ButovskyAdminError("Хаб вернул пустое событие")
            ...
            if event.get("event") == "tool_call":
                ...
                runtime = _runtime_context_from_event(event, topic_id=topic_id)
                logger.info(
                    "Butovsky Admin tool_call: admin=%s topic=%s request_id=%s tool_call_id=%s tool=%s",
                    telegram_id,
                    topic_id,
                    request_id,
                    tool_call_id,
                ...
                elif not mark_butovsky_admin_tool_call_started(request_id, tool_call_id):
                ...
                try:
                    ...
                    if tool_call_id:
                        clear_butovsky_admin_tool_call_started(request_id, tool_call_id)
            ...
            raise ButovskyAdminError(f"Неизвестное событие хаба: {event}")
    finally:
        if final_received:
            _clear_active_request(
                telegram_id,
...
async def run_dialog(
    ...
    *,
    ...
    """
    Performs a full cycle of dialogue with the Butovsky Admin agent.
    ...
    async with aiohttp.ClientSession(timeout=timeout) as session:
        effective_skill_id = butovsky_admin_skill_id_for_topic(topic_id, skill_id)
        ...
        agent_runtime_context = (
            None
            if effective_skill_id == BUTOVSKY_ADMIN_BROADCAST_SKILL_ID
            else _build_runtime_context_or_error(runtime_context_factory)
        )
        agent_message = (
            message
            if (
                effective_skill_id == BUTOVSKY_ADMIN_BROADCAST_SKILL_ID
        ...
        if not process_data:
            raise ButovskyAdminError("Хаб вернул пустой ответ на /process")
        ...
        async with satellite_lane_controller.cycle(key, request_id) as cycle:
            if cycle is None:
                raise ButovskyAdminError("Hub returned an already polled request", kind="protocol")
...
async def run_dialog_with_uploads(
    ...
    *,
    ...
    """Sends files to Butovsky Admin and waits for the final response from the agent."""
    if not uploads:
    ...
    async with aiohttp.ClientSession(timeout=timeout) as session:
        ...
        if not upload_data:
            raise ButovskyAdminError("Хаб вернул пустой ответ на upload")
...
def _latest_from_event(request_id: int, event: Optional[dict[str, Any]]) -> ButovskyAdminLatest:
    """Converts a hub snapshot into a local structure without tool_call."""
    if not event:
        return ButovskyAdminLatest(request_id=request_id)

    event_type = str(event.get("event") or "")
    if event_type == "final":
        return ButovskyAdminLatest(
            request_id=request_id,
            event=event_type,
            final=ButovskyAdminFinal(
                content=str(event.get("content") or ""),
                rich_markdown=event.get("rich_markdown"),
                viewer_url=event.get("viewer_url"),
                request_id=request_id,
    ...
    if event_type in {"status", "task_update"}:
        return ButovskyAdminLatest(
            request_id=request_id,
            event=event_type,
            progress=ButovskyAdminProgressEvent(
                event=event_type,
                content=str(event.get("content") or ""),
                slot=str(event.get("slot") or ""),
                cancel_button_text=event.get("cancel_button_text"),
    ...
    logger.warning(
        "Butovsky Admin latest ignored unsupported event: admin_request_id=%s event=%s",
        request_id,
        event_type,
    )
    return ButovskyAdminLatest(request_id=request_id)
...
) -> ButovskyAdminHubStatus:
    """Converts the hub /status response to a local structure."""
    if not data:
        return ButovskyAdminHubStatus(status="unsafe_unknown", request_id=request_id)
    status = str(data.get("status") or "unsafe_unknown")
    return ButovskyAdminHubStatus(
        status=status,
...
def _latest_from_hub_status(
    request_id: int,
    hub_status: ButovskyAdminHubStatus,
) -> ButovskyAdminLatest:
...
async def fetch_dialog_status(
    ...
    *,
    topic_id: int = BUTOVSKY_ADMIN_CHAT_TOPIC_ID,
    request_id: Optional[int] = None,
) -> Optional[ButovskyAdminHubStatus]:
...
async def start_new_chat(
    ...
    *,
    ...
    if not data:
        raise ButovskyAdminError("Хаб вернул пустой ответ на /new_chat")

    _validate_specialized_hub_response(data, effective_skill_id)
    result = ButovskyAdminNewChatResult(
...
async def _cancel_request(
    ...
    *,
    ...
    hub_status = _status_from_data(request_id, data)
    logger.info(
        "Butovsky Admin cancel result: admin=%s topic=%s request_id=%s status=%s",
        telegram_id,
        topic_id,
        request_id,
        hub_status.status,
    ...
    return ButovskyAdminCancelResult(
...
def _recovered_final_keyboard(topic_id: int, viewer_url: Optional[str]) -> Any:
    ...
    from bot.keyboards.admin_butovsky import butovsky_admin_agent_kb

    return butovsky_admin_agent_kb(
        topic_id,
...
async def _recover_one_active_dialog_on_startup(
    ...
    *,
    ...
    from bot.utils.butovsky_admin_delivery import send_butovsky_admin_final
        ...
        if hub_status is None or not hub_status.resume_allowed:
            if hub_status is not None and hub_status.status == "idle":
                ...
                if latest is not None and latest.final is not None:
                    await send_butovsky_admin_final(
                        bot,
                        chat_id=telegram_id,
            ...
            logger.info(
                "Butovsky Admin startup recovery skipped: admin=%s topic=%s request_id=%s status=%s",
                telegram_id,
                topic_id,
                request_id,
    ...
    except ButovskyAdminRequestStopped:
        return
    except Exception as e:
        logger.warning(
            "Butovsky Admin startup recovery failed: admin=%s topic=%s request_id=%s error=%s",
            telegram_id,
            topic_id,
            request_id,
            e,
...
async def recover_active_dialogs_on_startup(bot: Any) -> None:
    """Runs best-effort recovery of live Butovsky Admin tasks after a restart."""
    api_key = get_butovsky_admin_api_key()
    if not api_key:
        return
    active_requests = list_butovsky_admin_active_requests()
    ...
    try:
        ...
        if error.kind != "configuration":
            raise
        from bot.keyboards.admin_butovsky import butovsky_admin_chat_kb
        from bot.utils.butovsky_admin_errors import format_butovsky_admin_error
        ...
        for telegram_id in sorted({
            ...
            try:
                await bot.send_message(
                    chat_id=telegram_id,
                    text=format_butovsky_admin_error(error),
                    reply_markup=butovsky_admin_chat_kb(
                        BUTOVSKY_ADMIN_CHAT_TOPIC_ID
                    ),
                    parse_mode="HTML",
                )
            except Exception as delivery_error:
                logger.warning(
                    "Butovsky Admin configuration notice delivery failed: "
                    "admin=%s error=%s",
                    telegram_id,
                    delivery_error,
                )
