...
# Butovsky VPN — скрипт установки и управления
# Запуск: bash <(curl -sL https://raw.githubusercontent.com/DmitryBloomberg/ButovskyVPN/main/install.sh)
# 
# === АВТОМАТИЧЕСКИЙ ЗАПУСК (БЕЗ ДИАЛОГОВ) ===
#
# 1. Запуск прямо с GitHub (для чистой установки или если папки ещё нет):
# bash <(curl -sL https://raw.githubusercontent.com/DmitryBloomberg/ButovskyVPN/main/install.sh) install <BOT_TOKEN> <ADMIN_ID>
# bash <(curl -sL https://raw.githubusercontent.com/DmitryBloomberg/ButovskyVPN/main/install.sh) update [COMMIT_OR_BRANCH]
# bash <(curl -sL https://raw.githubusercontent.com/DmitryBloomberg/ButovskyVPN/main/install.sh) reset [COMMIT_OR_BRANCH]
# bash <(curl -sL https://raw.githubusercontent.com/DmitryBloomberg/ButovskyVPN/main/install.sh) rollback
#
...
INSTALL_DIR="/root/ButovskyVPN"
REPO_URL="https://github.com/DmitryBloomberg/ButovskyVPN.git"
VENV_DIR="$INSTALL_DIR/venv"
SERVICE_FILE="butovsky-vpn.service"
UPDATER_SERVICE_FILE="butovsky-vpn-updater@.service"
...
EOF
    ...
    if ! "$VENV_DIR/bin/python" -m bot.services.update_rollback install-service \
        --project-root "$INSTALL_DIR" \
        --service-name butovsky-vpn > /dev/null 2>&1; then
        ...
        if ! updater_unit_stage_dir=$(mktemp -d "/etc/systemd/system/.butovsky-updater-unit.XXXXXX"); then
...
[Unit]
Description=Butovsky VPN managed updater for snapshot %i
...
Type=oneshot
User=root
ExecStart=$VENV_DIR/bin/python $INSTALL_DIR/backup/pre_update/%i/service_runner.py service-request --project-root $INSTALL_DIR --snapshot-id %i --service-name butovsky-vpn
...
EOF
    ...
    if ! systemctl enable butovsky-vpn > /dev/null 2>&1; then
...
# Запуск сервиса
start_service() {
    systemctl start butovsky-vpn
    sleep 2

    if systemctl is-active --quiet butovsky-vpn; then
        print_ok "Бот запущен и работает!"
    else
        print_err "Бот не запустился. Проверьте логи:"
        echo "  systemctl status butovsky-vpn"
        echo "  journalctl -u butovsky-vpn -n 50"
    fi
}
...
# ============================================================
do_install() {
    print_header "🚀 Установка Butovsky VPN"

    # Проверяем, не установлен ли уже
    if [ -d "$INSTALL_DIR" ] && [ -d "$INSTALL_DIR/.git" ]; then
        print_warn "Butovsky VPN уже установлен в $INSTALL_DIR"
        if [ "$AUTO_MODE" = "1" ]; then
    ...
    print_header "Загрузка Butovsky VPN"
    ...
    echo -e "    ${CYAN}systemctl status butovsky-vpn${NC}   — статус"
    echo -e "    ${CYAN}systemctl restart butovsky-vpn${NC}  — перезапуск"
    echo -e "    ${CYAN}systemctl stop butovsky-vpn${NC}     — остановка"
    echo -e "    ${CYAN}journalctl -u butovsky-vpn -f${NC}   — логи"
}
...
do_soft_update() {
    ...
    if [ ! -d "$INSTALL_DIR/.git" ]; then
        print_err "Butovsky VPN не установлен в $INSTALL_DIR"
        return 1
    fi
...
do_hard_reset() {
    ...
    if ! systemctl stop butovsky-vpn; then
    ...
    if ! prepare_update_snapshot "$mode" "$target"; then
        if [ "$service_was_active" = "1" ]; then
            systemctl start butovsky-vpn || true
        fi
        release_update_operation_lock
        return 1
    fi
    ...
    if ! systemctl start butovsky-vpn; then
